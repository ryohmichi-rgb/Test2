...
| 機能 | 概要 | 主な画面 |
|------|------|----------|
| 名前だけで開始 | アカウント登録なしで学習開始 | StartPage |
| ホーム | 各機能への入口・今日のノルマ・成長曲線 | HomePage |
| 単元べつ演習 | 単元を選び、1問ずつ即フィードバック | GradesPage → PracticePage |
...
```mermaid
flowchart TD
    Start["StartPage 名前入力"] --> Home["HomePage ホーム"]
    Home --> Plan["PlanPage 今日のプラン"]
    Home --> Grades["GradesPage 学年→単元"]
    Grades --> Practice["PracticePage 単元演習"]
...
- ログイン状態は `localStorage`（`studentId` / `studentName`）で保持。
- 問題集の途中状態は `localStorage`（`problemset_<studentId>`）に保存し「続きから」再開。
- 開発中アクセス制限として `PasswordGate` を通す。
...
| 種別 | ファイル | 役割 |
|------|----------|------|
| ページ | `pages/*.tsx` | 各画面（Home / Grades / Practice / ProblemSet / Test / TestHistory / Review / Stats / Plan / Start） |
