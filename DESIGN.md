...
- **problems.hint / problems.solution** … `hint` は**解く前**に見るヒント（自分で押して開く）、
  ...
  `solution` は全152問に用意ずみ。空でも画面は落ちない（何も出ないだけ）。
...
中心を決めても選びようがないため、問題を増やして幅を作った（64→94→128→152問）。
...
- **答えに出てくる文字は `+-/0123456789:abx` の17種類だけ**（全152問を調査。小数点は使わない）。
...
- 選択式（`multiple_choice`）ではキーパッドも入力欄も出さない。
  **打てない答えは選択式にする**のが定石。キーパッドに無い文字（小数点・日本語・式そのもの）を
  答えにしたいときは選択式にすれば済む（全152問中25問。全単元に3問以上）。
