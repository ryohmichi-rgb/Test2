...
- **problems.hint / problems.solution** … `hint` は**解く前**に見るヒント（自分で押して開く）、
  ...
  `solution` は全128問に用意ずみ。空でも画面は落ちない（何も出ないだけ）。
...
中心を決めても選びようがないため、問題を増やして幅を作った（64→94→128問）。
**いまは全8単元が難易度3・4・5をそれぞれ3問以上持つ。** 上が薄いと、習熟した子は
難しい問題をすぐ解き切って以後は解き直し（満点の20%）ばかりになる。
実測: ある単元を10問連続正解した状態で5問出させると、平均難易度4.34・満点で解ける難4以上が6問残る。
...
- **答えに出てくる文字は `+-/0123456789:abx` の17種類だけ**（全128問を調査。小数点は使わない）。
  問題を足すときもこの範囲に収める。割り切れない答えは分数（`a/b`）にするか、割り切れる数にする。
...
- **安全ガードレール**（システムプロンプトで固定）：①その問題についてだけ答える ②**最終的な数値の答えは言わない**（丸投げしない・考え方に誘導） ③小中学生向けにやさしく・範囲を超えない ④学習と無関係な質問はやさしく断る ⑤短く・励ます口調。
  「学習」の範囲は**その問題の教科**で決まる（3.20「教科名の差し込み」）。決め打ちにすると教科を足したときに断られる。
...
キャラの人物像（PersonaCatalog）
  ↓
その役目の決まりごと（ClaudePersona::ROLE_RULES_TEMPLATE / ClaudeTeacher::ROLE_TEMPLATE）
  ↓
...
プロンプトには「◯◯の学習に関係のない話には乗りません」と書いてある。ここを
「算数・数学」と決め打ちすると、**教科を足したときにAIがその教科の質問を断る**。
そこで教科名だけをテンプレートの差し込み（`%{subject}`）にしている。

- **差し込みは `%{subject}` の1つだけ**。ほかに穴を作らない（キャラごとに書き換えられる
  余地を増やさないため）。文言そのものは `AiSafety::RULES_TEMPLATE` の1か所のまま。
- **先生に聞く** … 解いている問題の教科（`AiSafety.subject_label_for(problem)`）。
  国語の問題を開けば「国語のやさしい先生」になる。
- **この人に聞く** … 特定の問題に紐づかないので、いま出題できる教科ぜんぶ
  （`AiSafety.all_subjects_label` → 例「算数・数学・国語」）。無効な単元しかない教科は数えない。
- たどれないとき（単元なしの問題・教科ゼロ）は全教科名、それも無ければ「勉強」に落ちる。
...
> 学習と無関係な相談／個人情報／自己否定を引き出す／進路の断定を求める／年収を聞く／
