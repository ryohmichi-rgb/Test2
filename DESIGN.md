...
- **既定は閉じている。** 画面はすでに縦に長い（問題文＋入力欄＋キーパッド5段）ので、
  「✏️ メモを開く」を押したときだけ出す。既存の「ヒントを見る」「先生に聞く ▼」と同じ形。
- **保存しない。** 紙と同じ使い捨てで、問題を移れば新しい紙になる（親が `key={problem.id}` を
  渡して作り直す）。画像を localStorage に貯めると容量をすぐ使い切るうえ、メモは残す前提の
  ものではない。開いたり閉じたりしても線は消えない（同じ問題のあいだは保つ）。
...
| 共通 | `components/ScratchPad.tsx` | 手書きメモ（筆算・途中式）。問題を解く5画面で共用 |
