...
| ステータス | 6種の学力・目標サマリー・目標設定・参考ステータス・さびつき表示 | StatsPage |
...
| ステータス | 内容 | 主に伸びる教科 |
|-----------|------|---------------|
| 計算力 | 四則演算・分数・小数の正確さと速さ | 算数・数学 |
| 数的センス | 数の性質・規則性・比の理解 | 算数・数学 |
| 図形力 | 図形の性質・面積・体積の理解 | 算数・数学 |
| 文章読解力 | 文章題を式に落とし込む力 | 算数・数学／国語 |
| 論理力 | 順序立てて考え、式を組み立てる力 | 算数・数学／国語 |
| 語彙力 | 漢字・ことばの意味を知り、使い分ける力 | 国語 |

ステータスは `stat_types` テーブルで管理し、後から追加できる。**教科ごとに分けてはいない**。
「文章読解力」「論理力」のように教科をまたいで伸びる力があるので、単元側で
どのステータスを伸ばすかを選ぶ形にしている（`unit_stat_types`。1単元に複数可）。
...
- **problems.hint / problems.solution** … `hint` は**解く前**に見るヒント（自分で押して開く）、
  ...
  `solution` は全191問に用意ずみ。空でも画面は落ちない（何も出ないだけ）。
- **problem.problem_type** … `fill_in`（記述）または `multiple_choice`（選択）。選択の場合のみ `choices` を持つ。
  **採点は平文比較なので、選択式の `answer` は正解の選択肢の `text` と完全に一致させること**
  （画面は選んだ選択肢の文字列をそのまま送る）。`choices.is_correct` は管理画面の表示用で、
  採点そのものには使っていない。ずれると絶対に正解できない問題ができるので、seed に足したら必ず確かめる。
  正解の肢の**位置はばらけさせる**（いまは 25問で 6/6/7/6）。かたよると中身を読まずに位置で当てられる。
  並びは `has_many :choices, -> { order(:id) }` で seed に書いた順に固定している
  （DBの返す順まかせだと、決めた位置どおりに出るとは限らない）。
...
中心を決めても選びようがないため、問題を増やして幅を作った（64→94→128→152→191問）。
...
- **表**: `remark-gfm` で描く（素の Markdown に表は無い）。入れる前は
  `| 読み | 漢字 |` がそのまま画面に出ていた。読みと漢字の対応、つなぎことばの一覧など、
  並べてくらべる内容に向く。CSS（`.lesson-body table`）は前から用意してあった。
...
- **答えに出てくる文字は `+-/0123456789:abx` の17種類だけ**（記述式127問を調査。小数点は使わない）。
...
- 選択式（`multiple_choice`）ではキーパッドも入力欄も出さない。
  **打てない答えは選択式にする**のが定石。キーパッドに無い文字（小数点・日本語・式そのもの）を
  答えにしたいときは選択式にすれば済む（全191問中64問。全単元に3問以上）。
  **国語は全問が選択式**。漢字やひらがなの答えはキーパッドから打てないため。
...
| 共通 | `components/MarkdownView.tsx` | Markdown（GFM）＋数式（KaTeX）の描画。教材ページで使用 |
